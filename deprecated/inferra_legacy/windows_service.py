...
SERVICE_CLASS = "inferra_legacy.windows_service.InferraWindowsService"
...
def _build_serve_argv(config_path: Path, data_dir: Path | None) -> list[str]:
    ...
    if getattr(sys, "frozen", False):
        ...
        return argv
    cmd = [_python_executable(), "-m", "inferra_legacy.cli", "--config", str(config_path), "serve"]
    if data_dir is not None:
...
if win32serviceutil is not None:
    ...
    def _win32_exception_errno(exc: BaseException) -> int | None:
        """Best-effort Windows error code for pywin32 / OSError-style exceptions."""
    ...
    def try_run_frozen_windows_service() -> bool:
        ...
        scm_failed_connect = int(winerror.ERROR_FAILED_SERVICE_CONTROLLER_CONNECT)
        try:
            servicemanager.StartServiceCtrlDispatcher()
        except Exception as exc:
            # PyInstaller may load two copies of the pywin32 extension; ``exc`` might not be
            # isinstance(..., pywintypes.error) for the handler's imported class. Compare
            # ERROR_FAILED_SERVICE_CONTROLLER_CONNECT (1063) instead.
            code = _win32_exception_errno(exc)
            if code != scm_failed_connect:
                raise
            return False
        return True
