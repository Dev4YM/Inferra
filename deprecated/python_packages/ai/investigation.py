...
INVESTIGATION_MAX_AI_ATTEMPTS = 3
...
def _output_has_signal(output: InvestigationOutput) -> bool:
    return any(
        [
            output.headline.strip(),
            output.what_happened,
            output.why_it_matters,
            output.likely_causes,
            output.evidence,
            output.next_steps,
        ]
    )
...
def _normalize_investigation_output(output: InvestigationOutput) -> InvestigationOutput:
...
def _validation_error_reason(exc: ValidationError) -> str:
    ...
    return f"{location or 'payload'} {first.get('msg', 'is invalid')}"
...
async def run_investigation(
    ...
    *,
    ...
    warnings: list[str] = []
    for attempt in range(1, INVESTIGATION_MAX_AI_ATTEMPTS + 1):
        try:
            current_raw = await provider.chat(messages)
            raw_length = len(current_raw.strip())
            _LOGGER.info("investigation AI attempt=%s raw_length=%s", attempt, raw_length)
            if not current_raw.strip():
                raise ValueError("AI returned an empty response body")
            data = extract_json_object(current_raw)
            if not data:
                raise ValueError("AI returned no JSON object")
            validated = InvestigationOutput.model_validate(data)
            if not _output_has_signal(validated):
                raise ValueError("AI returned an investigation payload without meaningful content")
            return InvestigationResult(
                output=_normalize_investigation_output(validated),
                bundle=redacted,
                used_ai=True,
                provider_status=provider_status,
                trace=trace,
            ...
            reason = f"Attempt {attempt}: AI returned an invalid investigation payload: {_validation_error_reason(exc)}"
        except (OllamaError, ValueError) as exc:
            reason = f"Attempt {attempt}: {exc}"
        _LOGGER.warning("investigation AI recovery attempt=%s reason=%s", attempt, reason)
        warnings.append(reason)

    fallback_reason = (
        f"AI output was unusable after {INVESTIGATION_MAX_AI_ATTEMPTS} attempts. {warnings[-1]}"
        if warnings
        else "AI output was unusable."
    )
    return InvestigationResult(
        output=_deterministic_fallback(bundle, fallback_reason),
        bundle=redacted,
        used_ai=False,
        provider_status=provider_status,
        trace=trace,
        ...
        warnings=warnings,
        attempts=INVESTIGATION_MAX_AI_ATTEMPTS,
    )
