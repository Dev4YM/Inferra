...
from web._shared import hypothesis_to_dict as _hypothesis_to_dict, incident_to_dict as _incident_to_dict
...
def _latest_incident_id(runtime: InferraRuntime) -> str | None:
    latest = runtime.incident_store.list_incidents(
        state=[IncidentState.OPEN, IncidentState.INVESTIGATING, IncidentState.EXPLAINED],
        limit=1,
    )
    if not latest:
        return None
    return latest[0].incident_id
...
def build_investigation_router(deps: InvestigationDeps) -> APIRouter:
    ...
    def _resolve_mode(override: str | None) -> str:
        if override and override in {"operator", "expert", "developer"}:
            return override
    ...
    async def ai_ask(payload: dict[str, Any] = Body(...)) -> dict[str, Any]:
        ...
        if scope.startswith("incident:"):
            ...
            latest_incident_id = _latest_incident_id(runtime)
            if latest_incident_id:
                bundle = await _build_incident_bundle(deps, latest_incident_id, mode=mode, question=question)
                focus = f"incident:{latest_incident_id}"
            else:
