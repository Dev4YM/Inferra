...
- [x] Web UI: React control plane under `src/web/frontend` with Overview, Incidents, Systems, Evidence, AI Investigator, Workspace, Control, and Settings; packaged build output under `src/web/ui_dist`; build helpers `scripts/build-web.ps1` and `scripts/build-web.sh`; live `/ws` updates remain available for runtime streams; optional Playwright smoke (`pip install -e ".[ui]"` then `pytest`).
