...
def test_dependency_hypothesis_uses_service_graph_for_root_cause():
    ...
    assert top["score_breakdown"]["dependency_proximity"] == 1.0


def test_health_check_pass_penalizes_conflicting_dependency_hypothesis():
    graph = ServiceGraph()
    graph.add_relation("api", "postgres")
    postgres_health = _event('{"service":"postgres","level":"info","message":"health check passed healthy"}')
    api_timeout = _event('{"service":"api","level":"error","message":"timeout calling postgres"}', offset_seconds=1)

    hypotheses = SimpleHypothesisEngine(graph).generate("inc-test", [postgres_health, api_timeout])
    top = hypotheses[0]

    assert postgres_health.event_id in top["contradicting_events"]
    assert top["score_breakdown"]["contradiction_count"] == 1
    assert top["score_breakdown"]["contradiction_penalty"] == 0.85
...
def test_low_resource_metrics_contradict_resource_exhaustion():
    ...
    assert resource["score_breakdown"]["contradiction_penalty"] == 0.85
